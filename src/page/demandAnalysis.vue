...
<template>
          ...
          <template #default="{ node, data }">
            ...
            >
              <div class="treeTitle">
                <div
                  class="confirmed"
                  v-if="data.params && data.params.confirm"
                >
                  待确认
                </div>
                ...
                >
                  <template #item="{element}">
                    <div class="chooseCollapse" :title="element.name">
                      <img
                        :src="'src/assets/images/' + element.id + '.png'"
            ...
            <li>
              ...
              <div class="produceArea">
                ...
                >
                  ...
                  <span
                    @click="deleteProduce(params, index)"
              ...
              <teleport to="#teleport-produce">
                <el-drawer v-model="produceDrawer" size="15%" direction="rtl">
                  ...
                  >
                    ...
                    <el-button type="primary" @click="saveProduce"
                      >确认</el-button
                    >
                  </div>
                </el-drawer>
                ...
                >
                  ...
                  >
                    ...
                    <div class="inputChoose" v-show="item.type == '1'">
                      ...
                      <span @click="deleteGrid(item, item.i)" class="deleteGrid"
                        >×</span
                      >
                    </div>
                    <!-- 拖动tab表格进来 -->
                    <div class="tabTableChoose" v-show="item.type == '2'">
                      ...
                      <template #default> -->
                          ...
                          >
                            ...
                            <el-button type="primary" @click="addtabArea(item)"
                              >确认</el-button
                            >
                          </div>
                          <!-- </template> -->
                      ...
                      <span
                        style="z-index:9;cursor:pointer;position:absolute;right:2rem;top:0;line-height:16px;"
                        @click="
                          drawer = true;
                          tabNameInput = '';
                          tabColumeInput = '';
                          teleportCurrent = item.i;
                        "
                        title="添加tab页"
                        >+</span
                      >
                      <img
                        ...
                        style="z-index:9;height:15px;width:15px;cursor:pointer;position:absolute;right:10px;top:20%;"
                        @click="
                          drawer2 = true;
                          teleportColume = item.i;
                          item.tabTableParams.tabList.map((res) => {
                            if (res.name == item.tabTableParams.activeName) {
                              reserveDrawlist = JSON.parse(
                                JSON.stringify(res.columeList)
                              );
                              res.columeList.map((val) => {
                                val.content = '';
                              });
                            }
                          });
  ...
  <el-dialog v-model="isAdvice" title="总体建议" width="40%">
    ...
    <p style="margin-left: 10px; text-align: left;">1. 系统参与者分析章节缺少图片或表格说明。鉴于条目比较多，一般推荐有一张图或表来汇总“系统参与者分析”会更清晰一些。</p>
    <p style="margin-left: 10px; text-align: left;">2. 需求规格说明书附录中的“需求追踪表”最后一列的需求权重分布不合理，权重都是10。请确认所有权重都是最重要且一样重要吗？</p>
    <template #footer>
      <span class="dialog-footer">
    ...
    const methods = {
      ...
      append() {
        ...
        if (state.addTreedata.params) {
          newChild.params = JSON.parse(
            JSON.stringify(state.addTreedata.params)
          );
    ...
    onMounted(() => {
      window.addEventListener(
        ...
        async (e) => {
          ...
          if (e.data.status) {
            // console.log(e.data.status,'111111')
            await methods.getDaTree();
            ElNotification({
              title: "提示",
              type: "error",
            ...
            // console.log(state.treeData, state.params, cur);
            proxy.$http(proxy.$interface.saveDaTreeParams, {
              daTree: JSON.stringify(state.treeData),
              taskId: state.taskId,
            });
          }
