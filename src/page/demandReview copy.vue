...
<template>
  ...
  >
  <iframe id="iframe" class="iframe" src="http://192.168.162.117:3000/#/dr" frameborder="0"></iframe>
  ...
  (n) => {
    if (n.path === '/demandReview') {
      nextTick(() => {
        ...
        dom.onload = function () {
          loading.value = false
          myWindow.postMessage({ taskId: Cookies.get('taskId') }, 'http://192.168.162.117:3000')
